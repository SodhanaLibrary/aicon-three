...
export default {
  async apiFetch(endpoint, payload) {
    window.animaticonStore.dispatch(apiActions.clearApiResponse({name:endpoint}));
    const fetchArgs = {
      method: ApiEndpoints[endpoint].method,
      headers: Object.assign({},ApiEndpoints[endpoint].headers),
      body: JSON.stringify(payload)
    };
    let result = null;
    if(fetchArgs.method === 'get') {
      const query = qs.stringify(payload);
      await fetch(ApiEndpoints[endpoint].url+"?"+query).then(function(response) {
        return response.json();
      }).then(function(response) {
        result = response;
        window.animaticonStore.dispatch(apiActions.setApiResponse({name:endpoint, response}));
      });
    } else {
      await fetch(ApiEndpoints[endpoint].url, fetchArgs).then(function(response) {
        return response.json();
      }).then(function(response) {
        result = response;
        window.animaticonStore.dispatch(apiActions.setApiResponse({name:endpoint, response}));
  ...
  loadImageToCanvas(url, scope) {
      const img = new Image();
      img.crossOrigin = 'anonymous';
      img.id = new Date().getTime();
      console.log(img.id);
      img.onload = function() {
        const raster = new scope.Raster(img.id);
  ...
  changeImageSize({imgUrl, maxWidth = 110, maxHeight = 110}) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
          ...
          // const heightRetio = maxHeight / img.height;
          // const ratio = widthRatio > heightRetio ? heightRetio : widthRatio;
          const ratio = 0.5;
